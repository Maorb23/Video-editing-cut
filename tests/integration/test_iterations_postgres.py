...
def queue_legacy_preview(repo, edit_id, number):
    with repo._connect() as connection:
        repo._queue(connection, edit_id, number, "preview")
...
def test_revision_history_decisions_download_and_selected_approval(repository, tmp_path):
    ...
    assert response["decision_log"] == LOG and response["preview_status"] == "queued"
    ...
    assert repository.get_edit(edit_id)["active_iteration"] is None
...
def test_approval_precedes_rendering_and_compilation_does_not_queue_preview(repository):
    edit = _edit(repository)
    ...
    assert repository.claim("renderer", 300).kind == "render"
